<div>
    ...
    <x-hyco.table>
        ...
        <x-slot name="headingRight">
            <x-hyco.link wire:navigate href="{{ route('master.cash-account.form',0) }}" icon="plus" class="">
                Create
            </x-hyco.link>
        </x-slot>
        ...
        <x-hyco.table-tr>
            ...
            <td class="px-4 py-3 text-gray-600">
                ...
                <span class="bg-green-100 text-green-700 px-2 rounded">{{ $CashAccount->status }}</span>
                @else
                <span class="bg-red-100 text-red-700 px-2 rounded">{{ $CashAccount->status }}</span>
                @endif
            </td>
            <td class="px-4 py-3 text-gray-600">
            ...
            <td class="h-px w-px whitespace-nowrap px-4 py-3">
                ...
                <a href="javascript:void(0)" wire:click="delete({{ $CashAccount->id }})" class="text-xs bg-red-600 text-white px-3 py-1 rounded-lg">Del</a>
            </td>
        </x-hyco.table-tr>
        @empty
        <tr>
    ...
    <x-confirmation-modal wire:model.live="confirmDeletion">
        <x-slot name="title">
            {{ __('Delete Cash Account') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Are you sure you would like to delete this cash account?') }}
        </x-slot>
